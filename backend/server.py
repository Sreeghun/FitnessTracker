...
async def analyze_food_image(data: FoodImageAnalysis, current_user = Depends(get_current_user)):
    """Analyze food image using Google Gemini AI and estimate nutritional content"""
    try:
...
Be as accurate as possible with the nutritional estimates. If you cannot clearly identify food in the image, return an empty items array."""
        ...
        logger.info(f"Gemini response received: {response.text[:200] if response.text else 'No text'}")
        ...
        try:
            ...
            if "total_kcal" not in analysis and analysis["items"]:
                analysis["total_kcal"] = sum(item.get("kcal", 0) for item in analysis["items"])
            
            if "confidence" not in analysis:
                analysis["confidence"] = "medium"
            
            logger.info(f"Successfully analyzed {len(analysis['items'])} food items")
            return analysis
