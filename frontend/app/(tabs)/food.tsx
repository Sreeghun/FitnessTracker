...
export default function Food() {
  ...
  const handleAIFoodScan = async () => {
    console.log('🎯 handleAIFoodScan called');
    console.log('Platform:', Platform.OS);
    
    try {
      // For web platform
      if (isWeb) {
        console.log('📱 Web platform - opening file picker');
      ...
      // For mobile platforms - show options
      ...
      console.error('❌ Error in handleAIFoodScan:', error);
  ...
  const openCamera = async () => {
    try {
      ...
      Alert.alert('Camera Error', 'Failed to open camera. Please try gallery instead.');
    }
  };
  ...
  const analyzeFood = async (base64: string) => {
    ...
    try {
      ...
      if (errorDetail.includes('quota') || errorDetail.includes('billing') || errorDetail.includes('429')) {
        Alert.alert(
          'AI API Error',
        ...
        Alert.alert('Analysis Error', errorDetail || 'Failed to analyze food image. Please try again.');
      }
    } finally {
      ...
      <View style={styles.section}>
        ...
        {todayEntries.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptySubtext}>Tap the button above to scan food or add manually</Text>
          </View>
        ) : (
